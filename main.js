...
function resetExercise(num) {
    ...
    // Universal reset pentru text inputs (toate cele 7 exerciții folosesc text inputs)
    const dataMap = { 1: ex1Data, 2: ex2Data, 3: ex3Data, 4: ex4Data, 5: ex5Data, 6: ex6Data, 7: ex7Data };
    const d = dataMap[num];
    if (d) {
        d.forEach(item => {
            const el = document.getElementById(`ex${num}-${item.id}`);
            const fb = document.getElementById(`ex${num}-f${item.id}`);
            if (el) el.value = '';
            if (fb) {
